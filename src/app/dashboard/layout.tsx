...
import { LayoutDashboard, FileText, UploadCloud, CreditCard, Settings, LogOut, HelpCircle, User, ChevronDown, ShieldCheck, Menu, X } from 'lucide-react';
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-muted/20 flex flex-col md:flex-row overflow-x-hidden">
        ...
        {isMenuOpen && (
          <>
            ...
            <motion.aside
              ...
              className="fixed top-0 right-0 bottom-0 w-[80%] max-w-sm bg-card border-l border-border z-[80] md:hidden flex flex-col p-6 pt-20"
            >
                ...
                {navLinks.map((link) => {
                  ...
                  return (
                    <Link
                      key={link.href}
                      href={link.href}
                      className={`flex items-center gap-4 px-5 py-4 rounded-2xl transition-all font-bold ${isActive ? 'bg-primary/10 text-primary border border-primary/20' : 'text-muted-foreground hover:bg-muted'
                        }`}
                    >
                ...
                {bottomLinks.map((link) => {
                  ...
                  return (
                    <Link
                      key={link.href}
                      href={link.href}
                      className="flex items-center gap-4 px-5 py-4 rounded-2xl text-muted-foreground hover:bg-muted font-bold transition-all"
                    >
                      <Icon className="w-6 h-6" /> {link.label}
                    </Link>
                  );
                })}
                <button onClick={handleLogout} className="w-full flex items-center gap-4 px-5 py-5 rounded-2xl text-destructive hover:bg-destructive/10 font-bold transition-all mt-4">
                  <LogOut className="w-6 h-6" /> Exit Portal
                </button>
              </div>
            </motion.aside>
          </>
      ...
      <aside className="hidden md:flex w-72 flex-col bg-card/80 backdrop-blur-xl border-r border-border/40 h-[calc(100vh-80px)] fixed left-0 top-20 shadow-[10px_0_50px_-20px_rgba(0,0,0,0.1)] dark:shadow-none overflow-hidden z-40">
        ...
        <div className="flex-1 overflow-y-auto p-6 pt-6 no-scrollbar">
          ...
          <div className="pt-8 border-t border-border/50 space-y-2 mt-auto">
            ...
            <button onClick={handleLogout} className="w-full flex items-center gap-4 px-4 py-4 rounded-2xl text-destructive hover:bg-destructive/10 font-bold transition-all mt-4 group">
              <LogOut className="w-5 h-5 group-hover:scale-110 transition-transform" /> Exit Portal
            </button>
          </div>
        </div>
      </aside>
